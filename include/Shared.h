...
int32_t DeleteFile(const char *pathname, const struct stat *sbuf, int32_t type,
                   struct FTW *ftwb);
...
int8_t AddNewUser(char *restrict UserId, char *restrict Name,
                  char *restrict URL);
...
int8_t CurlMultiSetup(char **Buffers, char **Links, uint16_t Total, void *ProgressData);
