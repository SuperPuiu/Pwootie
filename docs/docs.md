...
If you get an error regarding a possible corruption for PwootieData.txt (pwootie file) then inspect the file and clean it manually, if applicable. If not, delete it completely and run Pwootie again to regenerate everything.

# Pwootie Building
To build from source you need the following libraries: `curl`, `libzip`. Make sure you have them downloaded before attempting to build the project, and also make sure you have `clang` or `gcc` installed. Once done, use `make release` and you'll get a new executable in the `bin` folder. Use `make` if you want to build a testing version of the bootstrapper. When running the program, ensure you have the `tar` command available as it's required to unpack WINE. If you want, you may install Pwootie as `pwootie` in `/usr/local/bin` by running `make install`.
